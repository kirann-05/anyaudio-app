...
import { getStreamUrl, search, scrape, getRecommendations } from './api';
...
class AudioEngine extends EventTarget {
  ...
  async autoplayNext() {
    ...
    console.log('🎵 Autoplay: Fetching real recommendations for', lastTrack.title);
    this.emit('loading', { loading: true });
    
    try {
      let recommendation;
      
      // Try to use true YouTube recommendations if possible
      const sourceUrl = lastTrack.audioUrl || lastTrack.audio_url || lastTrack.original_url || `https://www.youtube.com/results?search_query=${encodeURIComponent(lastTrack.title)}`;
      
      try {
        const recommendations = await getRecommendations(sourceUrl);
        if (recommendations && recommendations.length > 0) {
          // Pick the first true recommendation
          recommendation = recommendations[0];
          console.log('✅ Found true algorithmic recommendation:', recommendation.title);
        }
      } catch (recErr) {
        console.warn('Failed to fetch true recommendations, falling back to search', recErr);
      }
      
      // Fallback to old behavior if recommendation fails
      if (!recommendation) {
        const searchResults = await search(lastTrack.title);
        recommendation = searchResults[1] || searchResults[0];
      }
      
      if (recommendation) {
        showToast(`Up Next: ${recommendation.title}`, 'info');
        const userId = 'anonymous'; // Replace with actual user session if needed
