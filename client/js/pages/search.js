  ...
  try {
    ...
    results.forEach(res => {
      ...
      card.innerHTML = `
        ...
        <button class="btn btn-primary btn-sm w-full import-btn" style="margin-top:12px;">Listen Now</button>
      `;
      ...
      card.querySelector('.import-btn').addEventListener('click', async (e) => {
        ...
        const btn = e.target;
        btn.disabled = true;
        btn.innerHTML = '<div class="spinner-sm"></div> Importing...';
        
        try {
          const collection = await api.scrape(res.url, state.user.id);
          showToast('Imported and starting playback!', 'success');
